...
selected bar. The guide remains on the last selected bar after release. Date and value remain above
the plot in a larger readout. The zero-based side axis has
...
so fresh installs and existing layouts keep it off until explicitly enabled. Open **Today → Your Cards
→ edit/pencil → Hidden**, add the average, then save. A previous explicit Key Metrics opt-in migrates
once to Your Cards. Its arithmetic mean uses
...
retains its original opening and reset behavior. The average is available only under Your Cards.

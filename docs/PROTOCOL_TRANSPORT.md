...
NOOP's existing generic verifier (`verifyFrame` with family `whoop5`) also requires exact agreement between declared and supplied length, so truncation and trailing bytes are both rejected, but it is more permissive about minimum length and format selection. Its floor is **13 bytes** (`FrameLimits.whoop5MinimumFrameBytes`): the 8 header bytes including CRC16, the inner type byte, and the 4-byte CRC32 trailer. That floor is a deliberate empirical compatibility policy, not a structural limit: the envelope alone can represent a 12-byte frame with declared length 4 and an empty CRC-covered payload, and Goose's [`v5Payload`](https://github.com/b-nnett/goose/blob/ba9ae0280c9b5b9a1545baab8e944cb3b7563c62/GooseSwift/GooseBLEClient%2BParsing.swift#L890-L900) accepts that shape. The real fixtures committed here include 20-byte command responses and 24- and 32-byte frames, but no captured 12-byte boundary case; they prove valid traffic above the floor, not that 12 is invalid.

Reassemble complete frames first, distinguish outer formats, and validate the exact selected frame rather than letting a permissive checksum check imply support for another format. Parser errors for unsupported format, size, header CRC and body CRC are local failures, **not** command-result values on the wire.

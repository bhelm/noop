 ...
 *
 * The file is GENERATED by a standalone Swift run (`openspec/changes/frame-integrity/oracle-gen`) —
 * never hand-written, and never to be edited here. If this test goes red, either Kotlin moved or Swift
 * moved; find out which before regenerating.
 *
...
class FrameIntegrityOracleTest {
    ...
    /**
     * The classification rendered exactly as the generator renders it, so one string compares three
     * possible shapes (case, and for an end its unix + trim) without a second decoding rule.
     */

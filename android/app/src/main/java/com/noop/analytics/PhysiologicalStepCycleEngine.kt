...
internal object PhysiologicalStepCycleEngine {
    ...
    suspend fun compute(
        ...
        val recovered = ArrayList<IntelligenceEngine.PersistedPhysiologicalBoundary>()
        ...
        if (candidatePriorities.isNotEmpty()) {
            ...
            for (owner in candidatePriorities.map { it.first }.distinct()) {
                ...
                markersByOwner[owner] = repo.metricSeries(
                    ownerComputedId,
                    IntelligenceEngine.STEPS_CYCLE_ONSET_KEY,
                    markerFromDay,
                    markerToDay,
                ).mapNotNull { row ->
            ...
            for (item in IntelligenceEngine.persistedPhysiologicalBoundaries(
        ...
        for (window in windows) {
            ...
            val witnesses = stepWitnessByDay.entries.asSequence()
                ...
                .toList()
            val key = IntelligenceEngine.physiologicalStepCacheKey(
                owner = ownerIdentity,
            ...
            if (IntelligenceEngine.shouldRecountPhysiologicalSteps(cached?.key, key)) {
                var pages = 0
                var samples = 0
                var evaluated = false
        ...
        return Result(
            ...
            recoveredOwnerMarkerRows = IntelligenceEngine.recoveredStepMarkerRows(
                recovered,

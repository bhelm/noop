...
interface DeviceRegistryDao {
    ...
    @Query("DELETE FROM imuChunk WHERE deviceId = :deviceId") suspend fun deleteImuChunkCatalogFor(deviceId: String)
    ...
    @Query("UPDATE OR IGNORE imuChunk SET deviceId = :to WHERE deviceId = :from") suspend fun reKeyImuChunks(from: String, to: String)

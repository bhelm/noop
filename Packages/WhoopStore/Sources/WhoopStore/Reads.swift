...
extension WhoopStore {
    ...
    /// Per-day, per-owner change detector for every scored stream that
    /// [hrFingerprint(deviceId:from:to:)] does NOT witness.
    ///
    /// The whole-pass re-score gate answers "did anything change anywhere"; this answers "did THIS
    /// night's scored input change", which is the question the `analyzeRecent` per-day reuse cache
    /// asks. A history offload
    ...
    /// [hrFingerprint(deviceId:from:to:)] uses, materialising no rows. `COUNT(*)` is the
    /// load-bearing half: it moves when a
    ...
    public func dayStreamFingerprint(deviceId: String, from: Int, to: Int) async throws -> String {
        try syncRead { db in
            ...
            // query always returns exactly one row — same idiom as `hrFingerprint` above.
            guard let row = try Row.fetchOne(db, sql: """
                SELECT
